...
for i in range(1, all_u + 1):          #перебор всех юзеров, в данном цикле каждому их них
    completed = 0                   #будет присваиваться свое значение completed (количество выполненных заданий)
    for j in range(len(in_file)):
        if in_file[j]['completed'] and in_file[j]['userId'] == i:  #подсчет выполненных заданий
            completed +=1                                       #одного юзера
    element = {"task_completed":completed, "userId":i}  #внесение записей в словарь
